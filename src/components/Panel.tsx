...
// Every panel is full-bleed/edge-to-edge — "size" only controls how tall a
// beat reads (pacing), never width, so panels stack with zero gap between
// them like a real manhwa/webtoon strip.
...
/** Renders one chapter panel: a full-width, edge-to-edge image with its caption/bubble/sticker treatment overlaid directly on top — never below it, so the reading strip never breaks. */
export function Panel({ panel, index }: PanelProps) {
  ...
  return (
    <motion.div
      ...
      style={{ position: "relative", width: "100%", display: "block", lineHeight: 0 }}
    >
      <img
        src={imageUrl}
        srcSet={srcSet}
      ...
      {panel.variant === "photo-caption" && panel.caption && (
        <div
          style={{
            ...
            background: "linear-gradient(180deg, transparent, rgba(255,231,240,.92) 55%)",
          }}
        >
          <div style={{ fontFamily: fonts.body, fontStyle: "italic", fontWeight: 700, fontSize: 14.5, color: colors.inkPlum, textShadow: "0 1px 2px rgba(255,255,255,.6)" }}>
            {panel.caption}
          </div>
        </div>
      )}
