...
export function ChapterListPage({ chapters, isLoading = false, sortOrder = "chronological", onBack, onSelectChapter }: ChapterListPageProps) {
  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", background: colors.pink50, fontFamily: fonts.body, position: "relative" }}>
      <header style={{ position: "sticky", top: 0, zIndex: 10, background: "rgba(255,211,228,.92)", backdropFilter: "blur(6px)", padding: "16px 18px 10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <button onClick={onBack} style={{ width: 38, height: 38, borderRadius: 999, background: "#fff", border: `2px solid ${colors.pink300}`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 10px rgba(232,83,107,.18)", flex: "none", cursor: "pointer" }}>
          <ChevronLeftIcon />
        </button>
      ...
      <main style={{ position: "relative", zIndex: 2, padding: "8px 18px 60px", maxWidth: 520, margin: "0 auto" }}>
        ...
        {!isLoading && !isEmpty && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 8 }}>
            {sorted.map((chapter) => (
              <ChapterCard key={chapter.id} chapter={chapter} isNewest={chapter.number === newestNumber} onSelect={() => onSelectChapter?.(chapter)} />
            ))}
          </div>
        )}
      </main>
